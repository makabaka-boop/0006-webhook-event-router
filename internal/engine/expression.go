...
// EvalContext 使用给定上下文求值一个条件节点。
func EvalContext(ctx context.Context, c domain.Condition, root any) (bool, error) {
	if c.Op != "and" && c.Op != "or" && c.Op != "not" {
		if !strings.Contains("."+c.Path+".", ".*.") {
			return evalLeaf(c, root), nil
		}
		vals, err := newTraversal(ctx, c.Path, root).values()
		if err != nil {
			return false, err
		}
		return evalLeafValues(c, root, vals), nil
	}
	for _, child := range c.Children {
		matched, err := EvalContext(ctx, child, root)
		if err != nil {
			return false, err
		}
		if (c.Op == "and" && !matched) || (c.Op == "not" && matched) {
			return false, nil
		}
		if c.Op == "or" && matched {
			return true, nil
...
// EvalAllContext 判断一组条件是否全部命中并传递求值错误。
func EvalAllContext(ctx context.Context, conds []domain.Condition, root any) (bool, error) {
	return EvalContext(ctx, domain.Condition{Op: "and", Children: conds}, root)
}

func evalLeaf(c domain.Condition, root any) bool {
	vals := Lookup(c.Path, root)
	return evalLeafValues(c, root, vals)
}

func evalLeafValues(c domain.Condition, root any, vals []any) bool {
	switch c.Op {
	case "exists":

...
func (s *Store) queryRules(ctx context.Context, q string) ([]*domain.Rule, error) {
	...
	// Decode rule rows before loading their target associations.
	out := make([]*domain.Rule, 0, 8)
	for rows.Next() {
		r, err := scanRule(rows)
		if err != nil {
			rows.Close()
...
// scanRule converts one database row into the domain representation shared by
// list and single-rule lookups.
func scanRule(scanner interface{ Scan(...any) error }) (*domain.Rule, error) {
	var r domain.Rule
	var sid sql.NullInt64
	var cond, createdAt, updatedAt string
	if err := scanner.Scan(
		&r.ID,
		&r.Name,
		&sid,
...
func (s *Store) loadTargets(ctx context.Context, r *domain.Rule) error {
	...
	// The query result is already ordered and normalized for the rule.
	r.TargetIDs = ids
	return nil
}

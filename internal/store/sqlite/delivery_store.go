...
func (s *Store) ApplyDeliveryOutcome(ctx context.Context, id int64, outcome store.DeliveryOutcome) error {
	var deadAt any
	if outcome.Status == domain.DeliveryDead {
		deadAt = now()
	}
	_, err := s.db.Conn().ExecContext(ctx,
		"UPDATE deliveries SET status=?, attempts=?, next_retry_at=?, dead_at=?, last_error=?, updated_at=? WHERE id=?",
		outcome.Status, outcome.Attempts, nullableTime(outcome.NextRetryAt), deadAt, outcome.LastError, now(), id,
	)
	return err
}

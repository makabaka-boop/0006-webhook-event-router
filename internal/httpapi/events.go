...
func (s *Server) handleCreateEvent(w http.ResponseWriter, r *http.Request) {
	...
	matches, err := engine.MatchRulesContext(r.Context(), rules, req.SourceID, req.EventType, payloadBytes)
	if err != nil {
		respondErr(w, errs.New(errs.CodeInternal, "failed to match rules"))
		return
	}
	delivered := 0
	failed := 0

...
func (s *Server) handleCreateEvent(w http.ResponseWriter, r *http.Request) {
	...
	src, err := s.store.GetSource(r.Context(), req.SourceID)
	if err != nil {
	...
	// 幂等去重
	existing, err := s.store.GetByKey(r.Context(), req.SourceID, req.EventID)
	...
	deliveryCtx := dispatcher.NewDeliveryContext(r.Context())
	...
	matches := engine.MatchRules(rules, req.SourceID, req.EventType, payloadBytes)
	delivered := 0
	failed := 0
	...
	for _, m := range matches {
		for _, tid := range m.TargetIDs {
			...
			seenTarget[tid] = true
			dv, _ := s.dispatcher.Deliver(deliveryCtx, event, m.Rule, target)

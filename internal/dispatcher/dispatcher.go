...
func (d *Dispatcher) send(ctx context.Context, delivery *domain.Delivery, target *domain.Target, body string) {
	...
	if err != nil {
		d.recordFailedAttempt(ctx, delivery, att, err)
		return
	}
	...
	d.markFailed(ctx, delivery, "unexpected status "+http.StatusText(resp.StatusCode))
}

func (d *Dispatcher) recordFailedAttempt(ctx context.Context, delivery *domain.Delivery, att *domain.DeliveryAttempt, err error) {
	att.Status = domain.DeliveryFailed
	att.Error = err.Error()
	att.FinishedAt = time.Now()
	d.attempts.CreateDeliveryAttempt(ctx, att)
	d.markFailed(ctx, delivery, err.Error())
}
